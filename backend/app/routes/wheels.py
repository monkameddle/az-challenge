...
from ..models import Wheel, ChampionStatus
from ..security.auth import require_admin
from ..security.csrf import require_csrf
from ..ws import manager
import json, random

router = APIRouter(prefix="/wheels")

# Champion config (excluded list)
@router.get('/champion/config')
async def get_champion_config():
    async with SessionLocal() as s:
        rows = (await s.execute(select(ChampionStatus))).scalars().all()
        excluded = [r.key for r in rows if r.excluded]
        return {"excluded": excluded}

@router.put('/champion/config', dependencies=[Depends(require_admin), Depends(require_csrf)])
async def put_champion_config(body: dict):
    excluded: list[str] = body.get("excluded", [])
    async with SessionLocal() as s:
        # mark excluded flags
        for key in excluded:
