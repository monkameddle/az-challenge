...
export default function LoginForm() {
    ...
    return (
        <form className="max-w-sm mx-auto p-4 space-y-2" onSubmit={onSubmit}>
            <input className="border p-2 w-full" value={username} onChange={e => setU(e.target.value)}
                   placeholder="username"/>
            <input className="border p-2 w-full" value={password} onChange={e => setP(e.target.value)} type="password"
